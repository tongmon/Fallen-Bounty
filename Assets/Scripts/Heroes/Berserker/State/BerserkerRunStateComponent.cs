...
public class BerserkerRunStateComponent : StateComponent
{
    public BerserkerRunStateComponent(GameObject gameobject) : base(gameobject)
    {
    ...
    public override void Update()
    {
        ...
        if (data.m_target)
            distance_to_target = Vector2.Distance(data.m_physics_component.m_position, data.m_target.m_physics_component.m_position);

        if (data.m_point_target != null)
            distance_to_point = Vector2.Distance(((HeroPhysicsComponent)data.m_physics_component).m_bottom, data.m_point_target.Value);
