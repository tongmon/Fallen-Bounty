...
public class HeroPhysicsComponent : PhysicsComponent
{
    ...
    {
        // 영웅 이동 속도 더해주기
        // m_rigidbody.position += ((Hero)m_data).m_vec_direction.normalized * m_move_velocity * Time.deltaTime;

        // 영웅이 가야 하는 방향으로 현재 속도를 주면 어느 정도 속도가 이동 속도에 가해지는 지가 나옴 (벡터 중 이동 하려는 방향으로의 크기를 추출한다고 보면 됨.)
        Vector2 applied_movement = m_velocity.normalized * ((Hero)m_data).m_vec_direction.normalized;
