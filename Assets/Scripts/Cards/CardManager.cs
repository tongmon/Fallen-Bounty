...
public class CardManager
{
    ...
    public List<Card> MakeRandomCards()
    {
        ...
        {
            switch(m_cards[i].m_apply_category)
            {
                case "Everything":
                ...
                case "Ability":
                    {
                        List<Hero> heroes = hero_holder.GetHeroThatHaveTargetAbility(m_cards[i].m_apply_target);
                        ...
                        {
                            m_cards[i].m_apply_hero = hero_holder.m_heroes[UnityEngine.Random.Range(0, hero_holder.m_heroes.Count - 1)].m_name;
                            available_cards.Add(m_cards[i]);
                        }
                    }
                    break;
        ...
        for (int i = 0; i < cards_num; i++)
        {
            ...
            while (true)
            {
                ...
                if (!exsist_card.TryGetValue(candidate_card.m_name, out string name))
                {
                    exsist_card.Add(candidate_card.m_name); // 뽑은 카드 다시 못 뽑게 설정
    ...
    public List<(int, int)> OrderByQuality(ref List<Card> cards)
    {
        ...
        cards = cards.OrderBy(x => x.m_quality).ToList();
        int back_quality = cards[0].m_quality, back_quality_index = 0;
        for (int i = 1; i < cards.Count; i++)
        {
            if (back_quality != cards[i].m_quality)
            {
                quality_range.Add((back_quality_index, i - 1));
                back_quality = cards[i].m_quality;
